...
- `8081`：
  - 这是 mitmproxy 抓包代理端口
  - iPhone 的 Wi-Fi 代理里要填写这个端口
- `8765`：
  - 这是证书下载页面端口
  - 只用于在 iPhone Safari 中打开下载证书，不填到 Wi-Fi 代理里

操作顺序：

1. 先让电脑和 iPhone 处于同一局域网。
2. 不要求“Wi-Fi 名字完全相同”，但必须保证 iPhone 能直接访问到电脑当前显示的局域网 IP。
3. 打开桌面端，进入“抓包配置”，先点击“启动抓取服务”。
4. 确认页面里显示了类似 `代理地址: 192.168.x.x:8081` 和 `CA 下载地址: http://192.168.x.x:8765/`。
5. 在 iPhone 上打开：
   设置 -> WLAN -> 当前连接的 Wi-Fi -> 配置代理 -> 手动
6. 填写代理信息：
   服务器：电脑当前显示的局域网 IP
   端口：`8081`
7. 保存 Wi-Fi 代理配置。
8. 在 iPhone 上用 Safari 打开桌面端显示的 `CA 下载地址`，例如 `http://192.168.x.x:8765/`。
9. 在下载页点击 `下载 iOS 证书`，按系统提示下载描述文件。
10. 下载后到 iPhone：
    设置 -> 已下载描述文件
    然后完成证书安装。
11. 安装完成后，再到：
    设置 -> 通用 -> 关于本机 -> 证书信任设置
12. 找到 mitmproxy 相关证书，手动打开“完全信任”。
13. 完成后，先用 iPhone Safari 访问一个普通 HTTPS 网站确认联网正常。
14. 再打开目标页面/小程序，回到桌面端“抓取实时数据”页开始收集数据。

几个关键判断：

- 如果 iPhone 打不开 `http://<电脑IP>:8765/`
  - 先检查电脑和 iPhone 是否在同一局域网
  - 再检查桌面端“iOS 抓包检查”里 `本机 CA 端口监听` 是否成功
- 如果代理配置后 iPhone 完全无法上网
  - 先检查 `8081` 是否真的监听成功
  - 再检查证书是否已安装并完全信任
- 如果你已经完成证书安装和信任，后续一般不需要重复安装
  - 只有 mitmproxy CA 重新生成后，才需要重新下载安装和信任
